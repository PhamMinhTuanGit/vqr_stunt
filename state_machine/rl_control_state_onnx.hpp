...
private:
    ...
    std::shared_ptr<VqrWheelTestPolicyRunnerONNX> test_policy_;
    ...
    void UpdateRobotObservation(){
        ...
        buffer.projected_gravity = RmToProjectedGravity(buffer.base_rot_mat);
        ...
        buffer.cmd_vel_normlized = Vec3f(uc_ptr_->GetUserCommand().forward_vel_scale,
                                    uc_ptr_->GetUserCommand().side_vel_scale,
                                    uc_ptr_->GetUserCommand().turnning_vel_scale);
    ...
    void PolicyRunner(){
        ...
        while (start_flag_){
            ...
            if(state_run_cnt_%policy_ptr_->decimation_ == 0 && state_run_cnt_ != run_cnt_record){
                ...
                auto ra = policy_ptr_->GetRobotAction(rbs_[GetRbsReadIndex()]);
...
public:
    RLControlStateONNX(const RobotType& robot_type, const std::string& state_name, 
        ...
        test_policy_ = std::make_shared<VqrWheelTestPolicyRunnerONNX>("test_onnx");
