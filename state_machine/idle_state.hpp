...
private:
    ...
    bool JointDataNormalCheck(){
        ...
        Vec4f fl_lower4(fl_lower(0), fl_lower(1), fl_lower(2), -1e6f);
        Vec4f fl_upper4(fl_upper(0), fl_upper(1), fl_upper(2), 1e6f);
        Vec4f fr_lower4(fr_lower(0), fr_lower(1), fr_lower(2), -1e6f);
        Vec4f fr_upper4(fr_upper(0), fr_upper(1), fr_upper(2), 1e6f);
        joint_pos_lower << fl_lower4, fr_lower4, fl_lower4, fr_lower4;
        joint_pos_upper << fl_upper4, fr_upper4, fl_upper4, fr_upper4;
        for(int i=0;i<16;++i){
            ...
            float vel_limit = (i%4==3) ? cp_ptr_->wheel_vel_limit_ : cp_ptr_->joint_vel_limit_(i%4);
            if(std::isnan(joint_vel_(i)) || (joint_vel_(i)) > vel_limit + 0.1) {
                // std::cout << "joint vel " << i << " : " << joint_vel_(i) << " | " << vel_limit << std::endl;
                return false;
            }
        }
        return true;
