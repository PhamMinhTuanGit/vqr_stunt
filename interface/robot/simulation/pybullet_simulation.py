...
init(autoreset=True)
urdfPath = {
"vqr_wheel":    "/../../../vqr_description/vqr_urdf/urdf/VQRWheel.urdf",
}

ROBOT_ORDER_JOINT_NAMES = {
"vqr_wheel": [
...
# ROBOT order, per leg: same default leg pose as legged VQR, wheel=0.
initJointPos = {
"vqr_wheel":    [0.0, -0.65, 1.3, 0.0]*4,
}
...
class PyBulletSimulation:
    ...
    def __init__(self, robot_name, local_port=20001, ctrl_ip="127.0.0.1", ctrl_port=30010) -> None:
        ...
        for j in range(self.numOfJoints):
            ...
            nameToIdx[jointInfo[1].decode("UTF-8")] = j
        self.jointIdxList = [nameToIdx[name] for name in ROBOT_ORDER_JOINT_NAMES[self.robotName]]
        footNumList = [nameToIdx[name] for name in ROBOT_ORDER_JOINT_NAMES[self.robotName] if name.endswith("_WHEEL")]
        ...
        for idx, j in enumerate(self.jointIdxList):
            p.resetJointState(self.robot, j, targetValue=initJointPos[self.robotName][idx])
    ...
    def receiveJointCmd(self):
        ...
        while True:
            ...
            kp = struct.unpack(f'{n}f', data[0:n*4])
            targetPos = struct.unpack(f'{n}f', data[n*4:2*n*4])
            kd = struct.unpack(f'{n}f', data[2*n*4:3*n*4])
            targetVel = struct.unpack(f'{n}f', data[3*n*4:4*n*4])
            tau = struct.unpack(f'{n}f', data[4*n*4:5*n*4])
            self.kpCmd = np.array(kp).reshape(n, 1)
            self.kdCmd = np.array(kd).reshape(n, 1)
            self.jointPosCmd = np.array(targetPos).reshape(n, 1)
            self.jointVelCmd = np.array(targetVel).reshape(n, 1)
            self.tauCmd = np.array(tau).reshape(n, 1)
...
if __name__ == '__main__':
    ps = PyBulletSimulation("vqr_wheel")
