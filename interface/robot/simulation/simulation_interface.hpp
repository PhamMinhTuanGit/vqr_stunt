...
namespace interface{
    class SimulationInterface : public RobotInterface{
        ...
        public:
        SimulationInterface(const std::string& name, int dof_num=16):RobotInterface(name, dof_num){
        ...
        void ReceiveRobotData(){
            ...
            // 接收数据
            char buffer[1024]={0};
            // 11 header floats (run_time as double = 2 floats, rpy(3), acc(3),
            // omega(3)) + 16 DOF x {pos, vel, tau} = 11 + 48 = 59.
            float data[59]={0};
            ...
            while(start_thread_flag_){
                ...
                std::memcpy(data, buffer, 59*sizeof(float));
                run_time_ = ((double*)(data))[0];
                rpy_ = Eigen::Map<Vec3f>(data+2, 3);
                acc_ = Eigen::Map<Vec3f>(data+5, 3);
                omega_body_ = Eigen::Map<Vec3f>(data+8, 3);
                joint_pos_ = Eigen::Map<VecXf>(data+11, 16);
                joint_vel_ = Eigen::Map<VecXf>(data+27, 16);
                joint_tau_ = Eigen::Map<VecXf>(data+43, 16);
